...
| PostgreSQL 15   | 5432  | OPD queue, audit logs, OpenMRS data     |
...
```bash
cd guna_emr
cp .env.example .env    # Set POSTGRES_PASSWORD, OPENAI_API_KEY
docker-compose up -d
```
...
│
├── db/
│   └── init.sql                # PostgreSQL schema (queue + audit tables)
│
